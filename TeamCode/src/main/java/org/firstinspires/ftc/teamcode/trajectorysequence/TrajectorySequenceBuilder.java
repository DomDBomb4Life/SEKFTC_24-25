...
public class TrajectorySequenceBuilder {
   ...
   public TrajectorySequenceBuilder addTemporalMarker(double scale, double offset, MarkerCallback callback) {
       return this.addTemporalMarker(t -> scale * t + offset, callback);
   }
   ...
   public TrajectorySequenceBuilder addDisplacementMarkerOffset(double offset, MarkerCallback callback) {
   ...
   public TrajectorySequenceBuilder addDisplacementMarker(double scale, double offset, MarkerCallback callback) {
       return addDisplacementMarker(s -> scale * s + offset, callback);
   }
   ...
   public TrajectorySequenceBuilder turn(double angle, double maxAngVel, double maxAngAccel) {
       ...
       MotionProfile turnProfile = MotionProfileGenerator.generateSimpleMotionProfile(
               new MotionState(lastPose.getHeading(), 0.0, 0.0),
               new MotionState(lastPose.getHeading() + angle, 0.0, 0.0),
               maxAngVel,
               maxAngAccel
       );

       // Add TurnSegment with an empty marker list
   ...
   public TrajectorySequenceBuilder waitSeconds(double seconds) {
       ...
       // Add WaitSegment with an empty marker list
   ...
   public TrajectorySequenceBuilder addTrajectory(Trajectory trajectory) {
       ...
       currentDuration += trajectory.duration();
       currentDisplacement += trajectory.getPath().length();
       return this;
   }
   ...
   // Helper methods for marker conversion and projection...
   ...
   private List<TrajectoryMarker> convertMarkersToGlobal(
       ...
       // Implementation as per the reference code
       List<TrajectoryMarker> trajectoryMarkers = new ArrayList<>();
   ...
   private List<SequenceSegment> projectGlobalMarkersToLocalSegments(List<TrajectoryMarker> markers, List<SequenceSegment> sequenceSegments) {
       ...
       for (TrajectoryMarker marker : markers) {
           ...
           SequenceSegment newSegment = segment.withMarker(new TrajectoryMarker(segmentOffsetTime, marker.getCallback()));
   ...
   // Helper methods for displacementToTime and pointToTime
